    ...
    internal class OrderProcessor : IOrderManagementRepository
    {
        ...
        public bool CreateProduct(User user, Product product)
        {
            try
            {
                ...
                if (exist == true)
                {
                    ...
                    cmd.Connection = conn;
                    cmd.CommandType = System.Data.CommandType.Text;
                    cmd.CommandText = $"insert into product values('{product.ProductName}','{product.Description}','{product.Price}','{product.quantityInStock}','{product.Type}'); ";
                    conn.Open();
        ...
        public bool CreateUser(User user)
        {
            try
            {
                ...
                cmd.CommandText = "select SCOPE_IDENTITY();";
                object NewId = cmd.ExecuteScalar();
                Console.WriteLine($"your user id-{NewId}");
                if (rowcount > 0)
                {
                    return true;
                }
